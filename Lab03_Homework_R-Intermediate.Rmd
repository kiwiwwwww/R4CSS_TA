...
library(stringr)
b <- data.frame()
for (i in dir(path = "data/Lab03/")) {
   a <- read.csv(str_c("data/Lab03/" ,i ) )
   b <- rbind(a,b)
}
df_all<- b
df_all
...
### your code
score_addoil <- function(name,score) {
     if(!is.numeric(score)){
       print("小白目")
     }
     else{
     y = ceiling(sqrt(score) * 10)
     print(str_c(name,",你的期中考分數經過調整後是",y,"分,",sample(vector_addoil,1,replace = TRUE)))
}}
