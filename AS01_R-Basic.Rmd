...
vector_age[5]
# (3)
vector_state[seq(from=1,to=10,by=2)]
#kiwi的筆記：vector_state[c(1:10)%%2==1]
...
### your code
# (1)
vector_gender_first3_last3 <-c(df$gender[1:3] , df$gender[8:10])
#kiwi的筆記：c(head(df$gender,3) , tail(df$gender,3))所有資料裡面的前三跟後三
...
df_first20 <- df[1:20,]
# (2)
(subset(df_first20,gender=="female"))$age
# (3)
(subset(df_first20,patient_id%in%seq(from=1,to=20,by=2)))$released_date
# (4)
(subset(df_first20,age=="Above 50"&gender=="female"))$state
# (5)
(subset(df_first20,age!="Above 50"))$tourism_history
